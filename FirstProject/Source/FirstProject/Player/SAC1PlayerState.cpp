...
void ASAC1PlayerState::SetPlayerData(EPlayerJob Job, int32 AttackPoint, int32 ArmorPoint, int32 HP, int32 MP, int32 Level, int32 Exp, int32 Gold, float MoveSpeed, float AttackDistance)
{
	mJob = Job;
...
void ASAC1PlayerState::InitPlayerData(EPlayerJob Job)
{
	...
	if (IsValid(GameInst))
	{
		...
		// 위에서 구한 행이름으로 탐새하여 플레이어 정보를 얻어온다.
